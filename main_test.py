...
def get_argparser():
    ...
    parser.add_argument("--pixel_accuracy", action='store_true', default=True)
...
def get_dataset(opts):
    ...
    if opts.dataset == 'custom':
        ...
        train_mask_transform = transforms.Compose([
           ...
           # transforms.RandomResizedCrop(size=(512, 512), interpolation=transforms.InterpolationMode.NEAREST),
            # transforms.RandomHorizontalFlip(),
            #transforms.RandomRotation(degrees=(0, 360)),
            # transforms.ColorJitter(brightness=0.5, contrast=0.5, saturation=0.5, hue=0.5),
            transforms.ToTensor(),
...
def probs_print(opts, model, loader, device):
...
def main():
    ...
    iou_foreground_vals = [] # track IoU foreground values overtime
    iteration_vals = []
    fig, ax = plt.subplots()
    line, = ax.plot(iteration_vals, iou_foreground_vals)
    ax.set_xlim(0, 2000)  # number of iters
    ax.set_ylim(0, 1)  # foreground vals
    plt.show()

    
    def update_plot(new_x, new_y): # a func to update the plot 
        print("debug: update_plot function called")
    ...
    while True:  # cur_itrs < opts.total_itrs:
        ...
        for (images, labels) in train_loader:
            ...
            if (cur_itrs) % opts.val_interval == 0:
                ...
                iou_foreground_vals.append(val_score['IoU Foreground'])
                iteration_vals.append(cur_itrs)
                print("debug: " + str(iou_foreground_vals))
                print("debug: " + str(iteration_vals))
                
                # update_plot(iteration_vals, iou_foreground_vals)
                fig, ax = plt.subplots()
                line, = ax.plot(iteration_vals, iou_foreground_vals)
                ...
                if val_score['IoU Foreground'] > best_score:  # save best model
                    ...
                    save_ckpt('saved_models/inhouse_scratch_%s_%s_os%d_%s.pth' %
