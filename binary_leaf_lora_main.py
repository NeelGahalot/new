...
def get_dataset(opts):
    ...
    if opts.dataset == 'custom':
        ...
        train_dst = LeafDataset(root=opts.custom_data_path, image_set='train', img_transform=img_transform, mask_transform=mask_transform)
        val_dst = LeafDataset(root=opts.custom_data_path, image_set='val', img_transform=img_transform, mask_transform=mask_transform)
...
def main():
    ...
    #freeze_backbone_layers(model, 2)
    ...
    if opts.ckpt is not None and os.path.isfile(opts.ckpt):
        ...
        model.load_state_dict(torch.load('/content/drive/MyDrive/Training_Medicinal_Plants/saved_models/loratest_inhouse_india.pth'), strict=False)
        ...
        for (images, labels) in train_loader:
            ...
            if (cur_itrs) % opts.val_interval == 0:
                ...
                if val_score['IoU Foreground'] > best_score:  # save best model
                    ...
                    torch.save(lora.lora_state_dict(model), 'saved_models/loratest_inhouse_india.pth')
