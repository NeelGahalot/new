...
def binary_dice_loss(pred, target, bounding_boxes, weight_factor=2.0):
    ...
    for bbox in bounding_boxes:
        x1, y1, x2, y2 = bbox.tolist()  # Convert tensor to list
...
def validate(opts, model, loader, device, metrics, criterion, ret_samples_ids= None):
    ...
    with torch.no_grad():
        for i, (images, labels, boxes) in tqdm(enumerate(loader)):
...
def main():
    ...
    while True:  # cur_itrs < opts.total_itrs:
        ...
        for (images, labels, boxes) in train_loader:
            ...
            images = images.to(device, dtype=torch.float32)
            labels = labels.to(device, dtype=torch.long)
            boxes = boxes.to(device, dtype=torch.int32)
            ...
            loss = binary_dice_loss(outputs, labels, boxes)
