...
#                     out[out_name] = x
                    output_dimensions = x[0].size()[2:]  
                    upscaled_streams = [F.interpolate(stream, size=output_dimensions, mode='bilinear', align_corners=False) for stream in x[1:]]
                    x = torch.cat([x[0], *upscaled_streams], dim=1)
                    out[out_name] = x
                else:
                    out[out_name] = x
        return out
