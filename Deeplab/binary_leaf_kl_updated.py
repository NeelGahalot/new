...
class CombinedLoss(nn.Module):
    def __init__(self, temperature=2.0, kl_weight=1.0, dice_weight=1.0):
    ...
    def kl_divergence_loss(self, student_probs, teacher_probs):
        ...
        #student_probs /= student_probs.sum(dim=1, keepdim=True)
        #teacher_probs /= teacher_probs.sum(dim=1, keepdim=True)        
        # Compute log probabilities
        loss_pointwise = teacher_probs * (teacher_probs.log() - student_probs)
        ...
        return 1 - F.cosine_similarity(student_probs, teacher_probs).mean()
